...
--
--
-- Explicit-week challenge scheduling (kept alongside the legacy current-week RPC).
...
begin
  ...
  for week_offset in 0..clean_duration-1 loop
    ...
    insert into public.circle_weekly_challenges(
      ...
      stake_reward_id,stake_split_method
    ) values(
...
--
-- Native notification foundation. Production receives this through the matching migration.
...
CREATE FUNCTION public.unregister_native_push_device(installation_id_in text) returns void
language sql security definer
set search_path to 'public'
as $$
  delete from public.push_device_registrations
