-- Resetting a personal challenge now clears its attempt clock too.
--
-- begin_challenge_attempt() records when a round was first opened and keeps it
-- for the whole attempt, so re-entering cannot rewind the clock. That row
-- survived admin_reset_personal_challenge(), which deleted only the result --
-- so a replay after a reset resumed a clock that had been running since the
-- original first open. A round replayed in 30 seconds could be saved as two
-- hours, flooring its challenge score at the 20-point minimum.
--
-- admin_reset_all_stats() already cleared these for exactly this reason. The
-- narrower per-day reset now does the same for the rows it removes.

create or replace function public.admin_reset_personal_challenge(target_challenge_date date, target_game text DEFAULT NULL::text) RETURNS jsonb
