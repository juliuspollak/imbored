-- Stop scoring players against a difficulty that no longer exists.
--
-- Gridly's difficulty is admin-tunable per weekday - grid size, checkpoints,
-- walls, black holes and tunnel pairs are all seven-element arrays in
-- game_config, editable from the admin screen. Raising Sunday's difficulty
-- makes every earlier Sunday round incomparable, but the benchmark samples a
-- flat 90-day window, so it would keep scoring players against the easier board
-- for three months. Everyone would simply look worse, with nothing to say why.
--
-- generator_config is recorded on every round, so the sample now prefers rounds
-- played on the configuration currently in force for that weekday. Below eight
-- matching rounds it falls back to the whole window, because a stale reference
-- beats one built from three rounds. Games that record no config - geo and zoom
-- - match everything and are unaffected.
--
-- This also makes Gridly's backtracking usable as a scoring signal: comparing
-- backtracked cells only means something against boards of the same shape.
