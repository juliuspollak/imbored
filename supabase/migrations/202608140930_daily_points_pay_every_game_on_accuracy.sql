-- Daily points now pay every game on accuracy, not just Zoom.
--
-- award_game_points() had the accuracy rule hardcoded to game='zoom': the base
-- was scaled by correct/total and the speed bonus withheld on an imperfect
-- round, for that one game only. Geo now records the same correct_count and
-- total_count columns, and any future quiz will too, so the rule keys off
-- whether the row reports an answer count rather than off the game's name.
-- Games that report none (the solve-the-board puzzles) are unaffected.
