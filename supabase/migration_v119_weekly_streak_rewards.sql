-- Fairer gameplay rewards v119
--
-- 1. A streak earns one fixed bonus at each complete seven-day milestone,
--    rather than an increasingly large bonus on every day.
-- 2. Merely playing in challenge mode does not earn extra points. The
--    configured team challenge reward remains the winner's prize.
-- 3. The RPC returns the same breakdown that is stored with the transaction
--    so the result UI can explain the awarded total.

alter table public.reward_rules
  add column if not exists streak_weekly_bonus integer not null default 70;

update public.reward_rules
set challenge_bonus=0,
    streak_daily_bonus=0,
    updated_at=now()
where challenge_bonus<>0
   or streak_daily_bonus<>0;

alter table public.reward_rules
  drop constraint if exists reward_rules_streak_weekly_bonus_range;
alter table public.reward_rules
  add constraint reward_rules_streak_weekly_bonus_range
  check(streak_weekly_bonus between 0 and 500);
...
begin
  ...
  if benchmark_seconds is not null
