    ...
    return (
      <div className="rounded-2xl p-3" style={{ background: PANEL, border: approval ? "1px solid rgba(217,174,88,.30)" : "1px solid rgba(16,24,40,.08)", boxShadow: approval ? "0 8px 22px rgba(158,116,14,.08)" : "none" }}>
        <div className="flex items-center gap-3">
          ...
          {approval && (
            <button className="gloss-button" disabled={approvingId === player.id} onClick={() => handleApproval(player.id, true)} className="rounded-full px-3 py-2 text-xs font-semibold flex items-center gap-1 disabled:opacity-50" style={{ background: "rgba(22,163,74,.1)", color: "#15803D" }}>
              <CheckCircle2 size={13}/>{approvingId === player.id ? "Approving…" : "Approve"}
            </button>
          )}
          {!player.is_admin && <button className="gloss-button" onClick={() => setExpandedId(expanded ? null : player.id)} className="grid place-items-center rounded-full" style={{ width: 32, height: 32, background: "rgba(16,24,40,.045)" }} aria-label={`More actions for ${player.name}`}>
            <Ellipsis size={16}/>
          </button>}
        </div>
        {expanded && !player.is_admin && (
          <div className="flex flex-wrap gap-2 mt-3 pt-3" style={{ borderTop: "1px solid rgba(16,24,40,.07)" }}>
            {!approval && <button className="gloss-button" onClick={() => handleApproval(player.id, false)} className="rounded-full px-3 py-1.5 text-[11px] font-medium" style={{ background: "rgba(16,24,40,.05)" }}>Require approval</button>}
            <button className="gloss-button" onClick={() => player.is_blocked ? handleAccountAction("unblock", player) : setActionTarget({ type: "block", player, reason: "" })} className="rounded-full px-3 py-1.5 text-[11px] font-medium flex items-center gap-1" style={{ background: player.is_blocked ? "rgba(22,163,74,.1)" : "rgba(181,67,58,.08)", color: player.is_blocked ? "#15803D" : "#B5433A" }}>
              {player.is_blocked ? <RotateCcw size={11}/> : <ShieldBan size={11}/>}
              {player.is_blocked ? "Unblock" : "Block"}
            </button>
            <button className="gloss-button" onClick={() => handleToggleHidden(player)} className="rounded-full px-3 py-1.5 text-[11px] font-medium flex items-center gap-1" style={{ background: "rgba(16,24,40,.05)" }}><EyeOff size={11}/>{player.hidden_from_others ? "Show" : "Hide"}</button>
            <button className="gloss-button" onClick={() => setActionTarget({ type: "delete", player, reason: "" })} className="rounded-full px-3 py-1.5 text-[11px] font-medium flex items-center gap-1" style={{ background: "rgba(181,67,58,.08)", color: "#B5433A" }}><UserX size={11}/>Delete</button>
          </div>
        )}
      </div>
    );
