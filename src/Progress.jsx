...
export default function Progress({ onBack }) {
  ...
  return <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',sans-serif"}} className="p-4 pt-10 flex justify-center"><div className="w-full max-w-md">
    ...
    {loading?<p className="text-center text-sm opacity-40">Loading…</p>:<>
      ...
      <div className="flex gap-1 mb-3 p-1 rounded-xl" style={{background:"rgba(16,24,40,.05)"}}>{[["rewards","Rewards",Gift],["wish","Wish",Plus],["transfer","Transfer",Send]].map(([id,label,Icon])=>{const locked=id!=="rewards"&&!socialUnlocked;return <button key={id} disabled={locked} onClick={()=>setTab(id)} className="flex-1 rounded-lg py-2 text-xs font-semibold flex gap-1 justify-center items-center disabled:opacity-45" style={{background:tab===id?PANEL:"transparent",color:INK}}>{locked?<Lock size={12}/>:<Icon size={14}/>} {label}</button>})}</div>
