...
import { ArrowLeft, Star, Flame, Trophy, Gift, Send, Plus, ShieldCheck, ExternalLink, PartyPopper, X, Pencil, Save, Sparkles } from "lucide-react";
...
export default function Progress({ onBack }) {
  ...
  async function submitWish(e){e.preventDefault(); if(!wish.name.trim())return; const {error}=await supabase.from("reward_wishes").insert({player_id:user.id,...wish}); setMessage(error?.message||"Wish sent to admin"); if(!error){setWish({name:"",product_url:"",note:"",emoji:"🎁"});setWishEmojiTouched(false);} refresh();}
