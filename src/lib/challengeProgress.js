function groupChallengeCompletions(rows = []) {
  ...
  rows.forEach((row) => {
    const key = row.circle_challenge_id == null ? "personal" : String(row.circle_challenge_id);
    if (!grouped[key]) grouped[key] = new Set();
    grouped[key].add(row.circle_challenge_id == null ? row.game : row.challenge_date);
  });
  return grouped;
}
