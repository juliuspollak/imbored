...
test("past Circle results open score, rank and participant standings, with a winner only when final",()=>{ assert.match(home,/chooseHistoricalChallenge\(item\)/);assert.match(home,/winnerId=\{itemClosed \? item\.winner_id : null\}/);assert.match(home,/serverStandings=\{serverStandings\}/);assert.match(home,/item\.finisher_count/); });
