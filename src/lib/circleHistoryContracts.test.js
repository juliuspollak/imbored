...
test("missed ranked rounds remain server-closed while practice saves outside Circle scoring",()=>{ assert.match(schema,/Circle challenge rounds can only be played on their scheduled day/);assert.match(home,/Ranked play is closed\. Practice/);assert.match(home,/onPlayModeChange\?\.\("practice"\);onSelect\(gameId\)/); });
