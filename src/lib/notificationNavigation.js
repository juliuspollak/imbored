function notificationNavigation(data = {}) {
  ...
  if (!["circle", "circle_challenge", "daily_challenge", "competition_update", "challenge_result"].includes(route)) return null;
  const circleId = Number(data.circleId ?? data.circle_id);
  const challengeId = Number(data.challengeId ?? data.challenge_id);
  if (!Number.isSafeInteger(circleId) || circleId <= 0) return null;
  if (route === "circle" && (data.challengeId ?? data.challenge_id) == null) return { screen:"circles",circleId };
  if (!Number.isSafeInteger(challengeId) || challengeId <= 0) return null;
