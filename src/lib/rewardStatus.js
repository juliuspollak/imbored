export function rewardStatusText(reward, fallback = "No points awarded") {
  ...
  // daily_points_cap_reached is retained only so an older cached client keeps
  // showing something sensible: there is no global daily cap any more, and the
  // server always sends it as false. The per-game Practice limit is the only
  // throttle.
  if (reward.practice_limit_reached || reward.daily_limit_reached || reward.daily_points_cap_reached) {
