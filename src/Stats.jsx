...
import { Crown, Moon, Waypoints, Grid3x3, ArrowLeft, EyeOff, ChevronDown, ChevronUp, Clock, TriangleAlert, Lightbulb, Globe2, ZoomIn } from "lucide-react";
...
export default function Stats({ onBack }) {
  ...
  const totalsByGame = { queens: 0, tango: 0, zip: 0, minisudoku: 0, geo: 0, zoom: 0 };
  const byUser = {};
  modeRows.forEach((r) => {
    totalsByGame[r.game] = (totalsByGame[r.game] || 0) + 1;
    byUser[r.user_id] ||= { queens: 0, tango: 0, zip: 0, minisudoku: 0, geo: 0, zoom: 0, total: 0, rows: [] };
            ...
            {loading ? <p style={{ color: INK, opacity: 0.4 }} className="text-sm text-center py-8">Loading…</p> : modeRows.length === 0 ? (
              ...
              <>
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-6">
                  {["queens", "tango", "zip", "minisudoku", "geo", "zoom"].map((g) => { const Icon = GAME_ICONS[g]; return (
                      ...
                      <div key={p.userId} className="rounded-xl overflow-hidden" style={{ background: PANEL, border: "1px solid rgba(16,24,40,0.09)", opacity: p.profile.hidden_from_others ? 0.55 : 1 }}>
                        <div className="p-3 flex items-center gap-3">
                          <button className="flex flex-1 items-center gap-3 text-left min-w-0" onClick={() => setExpandedUserId(expanded ? null : p.userId)} aria-expanded={expanded}>
                            ...
                            <div className="flex-1 min-w-0"><div style={{ color: INK, fontWeight: 600 }} className="text-sm truncate">{p.profile.name}{p.profile.mood && <span style={{ fontWeight: 400, opacity: 0.5 }} className="text-xs"> · {p.profile.mood}</span>}</div><div className="flex gap-2 mt-0.5 flex-wrap">{["queens", "tango", "zip", "minisudoku", "geo", "zoom"].map((g) => p[g] > 0 ? <span key={g} style={{ color: GAME_COLORS[g] }} className="text-[10px] font-medium">{GAME_LABELS[g]} ×{p[g]}</span> : null)}</div></div>
