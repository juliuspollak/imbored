      ...
      <div className="w-full max-w-md">
        ...
        {composerOpen && <form onSubmit={create} className="rounded-3xl p-4 mb-5" style={{ background:PANEL,border:"1px solid rgba(47,111,237,.16)",boxShadow:"0 16px 38px rgba(47,111,237,.10)" }}>
          ...
          <div className="flex gap-2">
            <button className="gloss-button" type="button" onClick={() => setEmojiPickerOpen((open) => !open)} className="circle-icon-control rounded-2xl text-2xl shrink-0 flex items-center justify-center gap-0.5" style={{ width:54,background:"linear-gradient(145deg,#eef3ff,#fff)" }} aria-label="Choose circle icon">{emoji}<ChevronDown size={11} style={{ opacity:.35 }}/></button>
        ...
        {loading ? <div className="space-y-3 py-2" role="status" aria-label="Loading circles">{[0,1,2].map((item) => <div key={item} className="h-[96px] rounded-3xl animate-pulse" style={{ background:"linear-gradient(90deg,#E9ECF2,#F5F6F9,#E9ECF2)" }}/>)}</div> : circles.length === 0 ? <div className="rounded-3xl p-6 text-center" style={{ background:"#fff",border:"1px solid rgba(16,24,40,.07)" }}><Users size={24} className="mx-auto opacity-25"/><div className="text-sm font-semibold mt-2">No circles yet</div><div className="text-[11px] opacity-45 mt-1">Create the first circle when you’re ready.</div></div> : <div className="flex flex-col gap-3">{circles.map((circle) => {
          ...
          return <article key={circle.id} className="rounded-3xl p-4" style={{ background:PANEL,border:"1px solid rgba(16,24,40,.08)",boxShadow:isMine ? "0 12px 32px rgba(47,111,237,.07)" : "none" }}>
            <div className="flex items-center gap-3">
              <div className="grid place-items-center rounded-2xl text-2xl" style={{ width:48,height:48,background:"linear-gradient(145deg,#f0f4ff,#fff)" }}>{circle.emoji || "⭐"}</div>
              ...
              {manager ? <button className="gloss-button" onClick={() => { setRosterCircle(circle);setRosterQuery(""); }} className="rounded-full px-3 py-2 text-xs font-semibold flex items-center gap-1" style={{ background:"rgba(47,111,237,.09)",color:ACCENT }}><Users size={13}/>Manage</button>
                : isMine ? <button className="gloss-button" onClick={() => { setRosterCircle(circle);setRosterQuery(""); }} className="rounded-full px-3 py-2 text-xs font-semibold flex items-center gap-1" style={{ background:"rgba(47,111,237,.09)",color:ACCENT }}><Users size={13}/>Circle</button>
                : myRequest?.status === "pending" ? <span className="text-[10px] opacity-45">Requested</span>
                : <button className="gloss-button" disabled={profile?.hidden_from_others} onClick={() => request(circle.id)} className="rounded-full px-3 py-2 text-xs font-semibold" style={{ background:"rgba(47,111,237,.09)",color:ACCENT }}>Request to join</button>}
            </div>
          ...
          return <div className="circles-manage-page fixed inset-0 z-50 flex justify-center overflow-y-auto" style={{ background:BG }}>
            <div className="circles-manage-shell w-full max-w-md p-4 pt-6 flex flex-col" style={{ background:BG,minHeight:"100dvh" }}>
              <div className="flex items-center gap-3 mb-3">
                <button className="gloss-button" onClick={() => { setRosterCircle(null);setDeleteCircleTarget(null);setDeleteConfirmation(""); }} className="grid place-items-center rounded-full shrink-0" style={{ width:36,height:36,background:"rgba(16,24,40,.05)" }} aria-label="Back to circles"><ArrowLeft size={17}/></button>
                <div className="grid place-items-center rounded-2xl text-2xl" style={{ width:44,height:44,background:"linear-gradient(145deg,#eef3ff,#fff)" }}>{rosterCircle.emoji || "⭐"}</div>
              ...
              {member && <button className="gloss-button" onClick={() => { setInviteCircle(rosterCircle);setInviteQuery(""); }} className="w-full rounded-2xl px-4 py-3 mb-3 text-sm font-semibold flex items-center justify-center gap-2 text-white" style={{ background:ACCENT }}><UserPlus size={16}/>Invite a player</button>}

              {member && <div className="circle-challenges-panel rounded-3xl p-4 mb-3" style={{ background:"#fff",border:"1px solid rgba(16,24,40,.07)" }}>
                <div className="flex items-center gap-2 mb-3"><div className="grid place-items-center rounded-xl" style={{ width:34,height:34,background:"rgba(18,148,106,.09)",color:"#0B7C58" }}><CalendarDays size={15}/></div><div className="flex-1"><div className="text-sm font-bold">Challenges</div><div className="text-[10px] opacity-45">{rosterChallenges.length} this week</div></div>{owner && <button className="gloss-button" type="button" onClick={() => startNewChallenge(rosterCircle.id)} className="rounded-full px-3 py-2 text-[11px] font-semibold flex items-center gap-1" style={{ background:"rgba(47,111,237,.09)",color:ACCENT }}><Plus size={12}/>New</button>}</div>
              ...
              <div className="space-y-2 pr-0.5">
                {roster.map((member) => {
                  ...
                  return <div key={member.id} className="rounded-2xl p-3" style={{ background:isMe ? "rgba(47,111,237,.07)" : "#F8F9FC" }}>
                    <div className="flex items-center gap-3">
                      ...
                      {manager && !circleOwner && !isMe && <div className="flex gap-1">
                        <button className="gloss-button" disabled={!!moderationBusy} onClick={() => setTransferConfirmId(transferConfirmId === member.id ? null : member.id)} className="grid place-items-center rounded-full" style={{ width:32,height:32,background:"rgba(217,174,88,.14)",color:"#9A6A12" }} aria-label={`Make ${member.name} circle owner`} title="Make owner"><Crown size={13}/></button>
                        <button className="gloss-button" disabled={!!moderationBusy} onClick={() => toggleRewardApprover(rosterCircle,member)} className="grid place-items-center rounded-full" style={{ width:32,height:32,background:member.can_approve_rewards ? "rgba(18,148,106,.12)" : "rgba(16,24,40,.05)",color:member.can_approve_rewards ? "#12946A" : undefined }} aria-label={member.can_approve_rewards ? `Remove ${member.name} as reward approver` : `Make ${member.name} a reward approver`} title="Reward approver"><ShieldCheck size={13}/></button>
                        <button className="gloss-button" disabled={!!moderationBusy} onClick={() => moderateMember(rosterCircle,member,"remove")} className="grid place-items-center rounded-full" style={{ width:32,height:32,background:"rgba(16,24,40,.05)" }} aria-label={`Remove ${member.name}`} title="Remove"><UserMinus size={13}/></button>
                        <button className="gloss-button" disabled={!!moderationBusy} onClick={() => moderateMember(rosterCircle,member,"block")} className="grid place-items-center rounded-full" style={{ width:32,height:32,background:"rgba(181,67,58,.09)",color:"#B5433A" }} aria-label={`Block ${member.name}`} title="Block"><Ban size={13}/></button>
                      </div>}
                    </div>
                ...
                {manager && blocked.length > 0 && <div className="pt-3">
                  ...
                  {blocked.map((member) => <div key={member.user_id} className="flex items-center gap-3 rounded-2xl p-3 mb-2" style={{ background:"rgba(181,67,58,.055)" }}>
                    ...
                    <button className="gloss-button" disabled={!!moderationBusy} onClick={() => moderateMember(rosterCircle,{ id:member.user_id,name:member.member_name || "Player" },"unblock")} className="rounded-full px-3 py-2 text-[11px] font-semibold flex items-center gap-1" style={{ background:"#fff",color:ACCENT }}><RotateCcw size={12}/>Unblock</button>
                  </div>)}
                </div>}
