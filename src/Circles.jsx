                  ...
                  return <div key={member.id} className={`circle-member-row rounded-2xl p-3${isMe ? " circle-member-row--me" : ""}`} style={{ background:isMe ? "rgba(47,111,237,.07)" : "#F8F9FC" }}>
                    <div className="flex items-center gap-3">
                      ...
                      {manager && !circleOwner && !isMe && <div className="flex gap-1">
                        <button className="gloss-button circle-action-btn grid place-items-center rounded-full" data-variant="owner" disabled={!!moderationBusy} onClick={() => setTransferConfirmId(transferConfirmId === member.id ? null : member.id)} style={{ width:32,height:32,background:"rgba(217,174,88,.14)",color:"#9A6A12" }} aria-label={`Make ${member.name} circle owner`} title="Make owner"><Crown size={13}/></button>
                        <button className="gloss-button circle-action-btn grid place-items-center rounded-full" data-variant={member.can_approve_rewards ? "approver-active" : "approver"} disabled={!!moderationBusy} onClick={() => toggleRewardApprover(rosterCircle,member)} style={{ width:32,height:32,background:member.can_approve_rewards ? "rgba(18,148,106,.12)" : "rgba(16,24,40,.05)",color:member.can_approve_rewards ? "#12946A" : undefined }} aria-label={member.can_approve_rewards ? `Remove ${member.name} as reward approver` : `Make ${member.name} a reward approver`} title="Reward approver"><ShieldCheck size={13}/></button>
                        <button className="gloss-button circle-action-btn grid place-items-center rounded-full" data-variant="remove" disabled={!!moderationBusy} onClick={() => moderateMember(rosterCircle,member,"remove")} style={{ width:32,height:32,background:"rgba(16,24,40,.05)" }} aria-label={`Remove ${member.name}`} title="Remove"><UserMinus size={13}/></button>
                        <button className="gloss-button circle-action-btn grid place-items-center rounded-full" data-variant="block" disabled={!!moderationBusy} onClick={() => moderateMember(rosterCircle,member,"block")} style={{ width:32,height:32,background:"rgba(181,67,58,.09)",color:"#B5433A" }} aria-label={`Block ${member.name}`} title="Block"><Ban size={13}/></button>
                      </div>}
                    </div>
