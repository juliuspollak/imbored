          ...
          return <div className="circles-manage-page fixed inset-0 z-50 flex justify-center overflow-y-auto" style={{ background:BG }}>
            <div className="circles-manage-shell w-full max-w-md p-4 pt-6 flex flex-col" style={{ background:BG,minHeight:"100dvh" }}>
              <div className="flex items-center gap-3 mb-3">
                <button className="gloss-button grid place-items-center rounded-full shrink-0" onClick={() => { setRosterCircle(null);setDeleteCircleTarget(null);setDeleteConfirmation(""); }} style={{ width:36,height:36,background:"rgba(16,24,40,.05)" }} aria-label="Back to circles"><ArrowLeft size={17}/></button>
              ...
              <div className="space-y-2 pr-0.5">
                  ...
                  return <div key={member.id} className="rounded-2xl p-3" style={{ background:isMe ? "rgba(47,111,237,.07)" : "#F8F9FC" }}>
                    <div className="flex items-center gap-3">
                      ...
                      {manager && !circleOwner && !isMe && <div className="flex gap-1">
                        <button className="gloss-button grid place-items-center rounded-full" disabled={!!moderationBusy} onClick={() => setTransferConfirmId(transferConfirmId === member.id ? null : member.id)} style={{ width:32,height:32,background:"rgba(217,174,88,.14)",color:"#9A6A12" }} aria-label={`Make ${member.name} circle owner`} title="Make owner"><Crown size={13}/></button>
                        <button className="gloss-button grid place-items-center rounded-full" disabled={!!moderationBusy} onClick={() => toggleRewardApprover(rosterCircle,member)} style={{ width:32,height:32,background:member.can_approve_rewards ? "rgba(18,148,106,.12)" : "rgba(16,24,40,.05)",color:member.can_approve_rewards ? "#12946A" : undefined }} aria-label={member.can_approve_rewards ? `Remove ${member.name} as reward approver` : `Make ${member.name} a reward approver`} title="Reward approver"><ShieldCheck size={13}/></button>
                        <button className="gloss-button grid place-items-center rounded-full" disabled={!!moderationBusy} onClick={() => moderateMember(rosterCircle,member,"remove")} style={{ width:32,height:32,background:"rgba(16,24,40,.05)" }} aria-label={`Remove ${member.name}`} title="Remove"><UserMinus size={13}/></button>
                        <button className="gloss-button grid place-items-center rounded-full" disabled={!!moderationBusy} onClick={() => moderateMember(rosterCircle,member,"block")} style={{ width:32,height:32,background:"rgba(181,67,58,.09)",color:"#B5433A" }} aria-label={`Block ${member.name}`} title="Block"><Ban size={13}/></button>
                      </div>}
                    </div>
                ...
                {manager && blocked.length > 0 && <div className="pt-3">
                  ...
                  {blocked.map((member) => <div key={member.user_id} className="flex items-center gap-3 rounded-2xl p-3 mb-2" style={{ background:"rgba(181,67,58,.055)" }}>
                    ...
                    <button className="gloss-button rounded-full px-3 py-2 text-[11px] font-semibold flex items-center gap-1" disabled={!!moderationBusy} onClick={() => moderateMember(rosterCircle,{ id:member.user_id,name:member.member_name || "Player" },"unblock")} style={{ background:"#fff",color:ACCENT }}><RotateCcw size={12}/>Unblock</button>
                  </div>)}
                </div>}
